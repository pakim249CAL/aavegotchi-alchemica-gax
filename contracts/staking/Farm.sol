...
contract Farm is Ownable, ReentrancyGuard {
  ...
  // Keep track of which LP tokens are assigned to a pool
    ...
    nonReentrant
  {
    ...
    _updatePoolAndHarvest(msg.sender, _pid);
  ...
  function batchHarvest(uint256[] memory _pids) public nonReentrant {
    for (uint256 i = 0; i < _pids.length; ++i) {
      _updatePoolAndHarvest(msg.sender, _pids[i]);
    }
  }
  ...
  function _updatePoolAndHarvest(address _to, uint256 _pid) internal {
    PoolInfo storage pool = poolInfo[_pid];
    ...
    if (user.amount > 0) {
      ...
      rewardToken.transfer(_to, pendingAmount);

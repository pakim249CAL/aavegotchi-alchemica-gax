...
describe("GAX", function () {
  ...
  before(async function () {
    ...
    alchemicaImplementation = (
      await deployAlchemicaImplementation(owner)
    ).contract;
    fud = (
  ...
  describe("Factory", function () {
    it("should deploy GAX factory", async function () {
      let GAXFactory = await ethers.getContractFactory(
        "UniswapV2Factory"
      );
      factory = await GAXFactory.connect(owner).deploy(
        await address(owner)
      );
      expect(await address(factory)).to.not.be.equal(
        "0x0000000000000000000000000000000000000000"
      );
    });

    it("should deploy GAX router", async function () {
      let GAXRouter = await ethers.getContractFactory(
        "UniswapV2Router01"
      );
      router = await GAXRouter.connect(owner).deploy(
        await address(factory),
        "0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270"
      ); //WMATIC address
      expect(await address(router)).to.not.be.equal(
        "0x0000000000000000000000000000000000000000"
      );
    });
